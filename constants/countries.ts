// Country/region codes are persisted; names are only for display.
...
export const countries = countryCodes.map((code) => ({
  code, name: regionNames.of(code) || code,
})).sort((a, b) => a.name.localeCompare(b.name, "en"));

export function getCountryName(code?: string) {
  // Addresses created by the previous Philippines-only form have no country.
  return countries.find((country) => country.code === (code || "PH"))?.name || code || "";
}
